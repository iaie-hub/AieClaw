...
export async function resolveRuntimeWebTools(params: {
  ...
  const legacyXSearchSource: Record<string, unknown> | undefined = isRecord(sourceWeb?.x_search)
    ? sourceWeb.x_search
    : undefined;
  const legacyXSearchResolved: Record<string, unknown> | undefined = isRecord(resolvedWeb?.x_search)
