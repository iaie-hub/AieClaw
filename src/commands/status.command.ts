...
export async function statusCommand(
  ...
  const memoryValue = buildStatusMemoryValue({
    ...
    warn,
    muted,
    resolveMemoryVectorState: (value: unknown) =>
      resolveMemoryVectorState(value as { enabled: boolean; available?: boolean }),
    resolveMemoryFtsState: (value: unknown) =>
      resolveMemoryFtsState(value as { enabled: boolean; available: boolean }),
    resolveMemoryCacheSummary: (value: unknown) =>
      resolveMemoryCacheSummary(value as { enabled: boolean; entries?: number }),
  });
  ...
  const sessionsRows = buildStatusSessionsRows({
    recent: summary.sessions.recent.map((s) => ({
      ...s,
      age: s.age ?? 0,
    })),
    ...
    formatTokensCompact: (value) =>
      formatTokensCompact(value as unknown as Parameters<typeof formatTokensCompact>[0]),
    formatPromptCacheCompact: (value) =>
      formatPromptCacheCompact(value as unknown as Parameters<typeof formatPromptCacheCompact>[0]),
    muted,
  });
  ...
  const lines = await buildStatusCommandReportLines({
    ...
    muted: theme.muted,
    renderTable: renderTable as (input: {
      width: number;
      columns: Array<{ key: string; header: string; [k: string]: unknown }>;
