...
export function attachGatewayWsConnectionHandler(params: AttachGatewayWsConnectionHandlerParams) {
  ...
  wss.on("connection", (socket, upgradeReq) => {
    ...
    logWsControl.info(
      `connect.challenge sent conn=${connId} remote=${remoteAddr ?? "?"} handshakeTimeoutMs=${resolvePreauthHandshakeTimeoutMs({ configuredTimeoutMs: params.preauthHandshakeTimeoutMs })}`,
    );
    send({
      type: "event",
    ...
    const handshakeTimer = setTimeout(() => {
      if (!client) {
        ...
        logWsControl.warn(
          `handshake timeout conn=${connId} peer=${endpoint ?? "n/a"} remote=${remoteAddr ?? "?"} elapsed=${Date.now() - openedAt}ms`,
        );
        close();
      }
    }, handshakeTimeoutMs);
