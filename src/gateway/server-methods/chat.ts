...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    console.log(`[gateway:chat.send] Loading session entry for rawSessionKey=${rawSessionKey}...`);
    ...
    try {
      console.log(`[gateway:chat.send] Registering abort controller...`);
...
const chatSendTraceAttributes = {
      ...
      // The webchat send path has no before-agent-run gate hook; eager transcript
      // emission is always safe. emitUserTranscriptUpdate writes the accepted
      // user turn to disk so gateway watchers see it before model startup.
      const hasBeforeAgentRunGate = false;
      const emitUserTranscriptUpdate = persistGatewayUserTurnTranscriptBestEffort;
          ...
          const dispatchResult = await dispatchInboundMessage({
            ...
            replyOptions: {
              ...
              onReasoningStream: ({ text }) => {
                ...
                const delta =
                  text && text.startsWith(prior) ? text.slice(prior.length) : (text ?? "");
                if (delta) {
