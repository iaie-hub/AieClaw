...
export async function resetModelCatalogCacheForTest(): Promise<void> {
  const { resetModelCatalogCacheForTest } = await loadGatewayModelCatalogModule();
      ...
      cachedLlmFacade ??= {
        complete: async (params: any) => {
          ...
          try {
            ...
            runtimeLlmLog.info(
              `plugin llm completion - purpose: ${params.purpose || "none"}, agentId: ${result.agentId || "default"}, model: ${result.model || "default"}`,
              {
                systemPrompt,
                requestMessages,
            ...
            runtimeLlmLog.error(
              `plugin llm completion failed - purpose: ${params.purpose || "none"}: ${err.message || String(err)}`,
              {
                systemPrompt,
                requestMessages,
              },
...
export async function startGatewayServer(
  ...
  log.info(`startGatewayServer: starting on port ${port}...`);
  ...
  log.info("startGatewayServer: startup auth and secrets prepared");
  ...
  try {
    ...
    // and causing WebSocket 1006 failures. Moved to post-listen background task below.
    ...
    const wrappedBroadcast: typeof broadcast = (event, payload, opts) => {
      ...
      if (filtered === null || filtered.size === clients.size) {
