  ...
  try {
    ...
    if (!minimalTestGateway) {
      if (deferredConfiguredChannelPluginIds.length > 0) {
        log.info(
          `reloading ${deferredConfiguredChannelPluginIds.length} deferred channel plugin(s)...`,
        );
    ...
    log.info("attaching WebSocket handlers...");
    ...
    log.info("calling startListening...");
    await startListening();
    log.info("http server bound, port is now accepting connections");
