...
// ── Process-level AuthStorage + ModelRegistry cache ──────────────────────────
// Key is agentDir; uses mtime for invalidation, no TTL.
// auth-profiles.json can be modified externally (user runs openclaw auth), mtime change invalidates.
...
export function discoverAuthStorage(
  ...
  if (cached && !options) {
    // Pure mtime check, no TTL: if file unchanged, return cached
  ...
  // Cache miss or mtime changed, execute original logic
  const credentials =
  ...
  if (!options) {
    // Pre-build modelRegistry and write to cache
...
export function discoverModels(
  ...
  if (cached && cached.authStorage === authStorage && !options) {
    // Pure mtime check
  ...
  // Cache miss or mtime changed, rebuild registry
  const modelRegistry = createOpenClawModelRegistry(
    authStorage,
    path.join(agentDir, "models.json"),
    agentDir,
  ...
  if (!options) {
    // Update cache entry with new modelRegistry and modelsJsonMtimeMs
