...
- [x] 1. 扩展数据库 Schema（`aiemas/src/store/database.ts`）
...
- [x] 3. 扩展 `session-manager.ts`（`aiemas/src/gateway-bridge/session-manager.ts`）
  ...
  - 新增 `unarchiveSession(db, sessionKey, callerUserId): void`：校验调用者为 owner，将 `session_ownership.archivedAt` 重置为 NULL
...
- [x] 4. 新增 LLM 摘要服务（新文件 `aiemas/src/gateway-bridge/summary-llm.ts`）
...
- [x] 5. 扩展 `bridge.ts`（`aiemas/src/gateway-bridge/bridge.ts`）
  ...
  - 新增 `unarchiveSession(params: { sessionKey, callerUserId }): Promise<{ ok: true } | { ok: false; code: string; message: string }>`：校验调用者为 Session_Owner，调用 `sessionManager.unarchiveSession`，成功后调用 `pushSessionUnarchived`
  - 新增 `pushSessionUnarchived(sessionKey, payload, connectedUsers, sendToClient): void`：向该会话所有在线成员推送 `event:session.unarchived`
  ...
  - 在 `_isSessionAccessMethod` 中新增 `session.unarchive`
  - _需求：3.1、3.2、3.4、3.6、3.11、3.13、4.1、4.3、4.4、5.5_

  - [x] 5.1 为归档后拒绝发送消息编写属性测试
    - **属性 5：归档后拒绝发送消息**
    - **验证：需求 3.4、3.5**
...
- [x] 6. 扩展 `mas4s-gateway-plugin.ts`（`aiemas/src/gateway-bridge/mas4s-gateway-plugin.ts`）
  ...
  - 新增 `session.unarchive` 路由：提取 `callerUserId`，调用 `bridge.unarchiveSession`，成功后调用 `bridge.pushSessionUnarchived`
...
- [x] 10. 新增前端归档接口（新文件 `aiemas/ui/mas4s/src/gateway/session-archive.ts`）
  ...
  - 实现 `unarchiveSession(client, sessionKey): Promise<void>`：调用 `client.request("session.unarchive", { sessionKey })`，失败时抛出含错误码的 Error
...
- [x] 11. 更新 SessionSidebar 归档标识（`aiemas/ui/mas4s/src/components/session-sidebar.ts`）
...
- [x] 12. 更新 ChatView 输入框禁用逻辑（`aiemas/ui/mas4s/src/views/chat-view.ts`）
...
- [x] 15. 编写属性测试文件（`aiemas/src/gateway-bridge/session-collaboration.property.test.ts`）
  ...
  - [x] 15.7 属性 10：归档与启用互逆性
    - **属性 10：归档与启用互逆性**
    - **验证：需求 3.11、3.14**
    - 断言 `archiveSession` 后 `isSessionArchived` 返回 `true`；随后 `unarchiveSession` 后 `isSessionArchived` 返回 `false`，`interceptMethod("chat.send", ...)` 不再返回 `SESSION_ARCHIVED`
    - 标签：`// Feature: mas4s-session-collaboration, Property 10: 归档与启用互逆性`
