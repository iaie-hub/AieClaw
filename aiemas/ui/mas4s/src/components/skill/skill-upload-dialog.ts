 ...
 */
@customElement("skill-upload-dialog")
export class SkillUploadDialog extends LitElement {
  ...
  willUpdate(changed: Map<string, unknown>) {
    ...
    if (changed.has("skillName") && this.skillName) {
      this._uploadName = this.skillName;
    }
    ...
    return html`
      <div class="overlay" @click=${this._onOverlayClick}>
        <div class="dialog-card" role="dialog" aria-modal="true" aria-labelledby="upload-title">
          <h3 id="upload-title">上传到 SkillHub</h3>
          <p class="subtitle">打包并上传技能共享至 AgentRegistry 后端</p>

          <!-- Input: Skill Name -->
          <div class="form-group">
            <label for="skill-name-input">技能名称</label>
            <input
              id="skill-name-input"
              type="text"
          ...
          <div class="form-group">
            <label for="skill-desc-input">描述信息 (SKILL.md)</label>
            <textarea
              id="skill-desc-input"
              rows="3"
...
declare global {
  interface HTMLElementTagNameMap {
    "skill-upload-dialog": SkillUploadDialog;
  }
}
