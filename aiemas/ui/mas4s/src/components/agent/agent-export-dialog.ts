...
/**
 * 导出智能体对话框 — 展示工作区文件列表供用户勾选。
 * 触发 confirm({ items: string[] }) 或 cancel 事件。
 */
@customElement("agent-export-dialog")
export class AgentExportDialog extends LitElement {
  ...
  private _onConfirm = () => {
    ...
    const baseName = this.agentName.trim() || "agent-export";
    ...
    return html`
      <div class="overlay" @click=${this._onOverlayClick}>
        <div class="dialog-card" role="dialog" aria-modal="true" aria-labelledby="export-title">
          <h3 id="export-title">导出智能体</h3>
          <p class="subtitle">选择要导出的文件和目录（${this.agentName}）</p>
          <div class="toolbar">
...
declare global {
  interface HTMLElementTagNameMap {
    "agent-export-dialog": AgentExportDialog;
  }
}
