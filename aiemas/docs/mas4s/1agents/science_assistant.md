...
2. **逆向流转**：仅通过明确的 `[回退]` 指令触发，携带 `target_stage` 与 `fatal_error_log`。
...
| 指令             | 触发条件                   | 状态路由                                                                          |
| :--------------- | :------------------------- | :-------------------------------------------------------------------------------- |
| **`[继续]`**  | 当前输出完全符合预期       | 冻结并落盘当前阶段数据，状态机指针跃迁至下一阶段                                  |
| **`[重试]`**    | 输出有瑕疵或需补充约束     | 拦截落盘，清空当前结果，状态机原地重启。携带 `human_feedback` / `new_constraints` |
| **`[回退]`** | 当前困境源于上游根本性错误 | 废弃临时态数据，强制退回指定历史阶段。携带 `target_stage` 与 `fatal_error_log`    |
...
**辅助步骤：课题重构** — 当 `[回退]` 触发时，调用 `mas4s-topic-pivot` 解析失败逻辑、吸收反馈、孕育新种子，重回步骤 1。
...
| **HITL 网关**   | 提供 Go/No-Go 建议，引导用户进行 `[继续]` / `[重试]` / `[回退]`。 |
...
- `[继续]`：确认底座逻辑完备 → 流转至**实验设计 (D)**。
- `[重试]`：调整关键词、扩充检索范围 → 原地循环。
- `[回退]`：致命抢发或证据链断裂 → 携带 Pivot 建议打回**课题对齐 (T)**。
...
- `[继续]`：冻结方案蓝图与评估协议 → 流转至**代码实现 (I)**。
- `[重试]`：调整基线、指标或消融设计 → 原地循环。
- `[回退]`：方案不可行或已被完全覆盖 → 回退至**课题对齐 (T)** 或**文献底座 (L)**。
...
- `[继续]`：确认所有资产与代码就绪 → 流转至**实验执行 (E)**。
- `[重试]`：修复复现偏差、Bug 或环境问题 → 原地循环。
- `[回退]`：实现级不可行 → 回退至**实验设计 (D)**。
...
- `[继续]`：确认实验完整、结果无误 → 流转至**论文写作 (W)**。
- `[重试]`：通常不适用。
- `[回退]`：结果无效 → 回退至**代码实现 (I)**，甚至**实验设计 (D)**。
...
- `[继续]`：逐字审阅所有主张、映射与格式 → 产出最终送审稿，流程结束。
- `[重试]`：框选部分重写 → 原地循环，不得改动阶段 5 客观结果。
- `[回退]`：论证链缺口或缺失关键实验 → 回退至**实验执行 (E)**，甚至**实验设计 (D)**。
...
- 所有 `[重试]` 在当前阶段形成内部闭环，不触发指针跃迁。
- `[回退]` 携带 `fatal_error_log` 与 `target_stage`，允许跨多阶段跳回。
- 流转至论文写作的 `[继续]` 标志着系统完成最终送审稿产出。
