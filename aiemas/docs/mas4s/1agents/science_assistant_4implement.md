...
4. **冰封实验现场（不可变执行）**：
   - 最终，通过运行 **`Dockerfile`** (来自步骤 4)，将上述所有的脚本流、库依赖版本和数据读取逻辑打包冻结为不可变容器。
   - **重要执行路径说明**：虽然 Dockerfile 在此阶段产出，但为了确保实验结果能回传至对应的运行记录（Run ID），实际的 `docker build` 与 `docker run` 操作必须在 **阶段 5（实验执行）的任务工作空间** 中进行。
