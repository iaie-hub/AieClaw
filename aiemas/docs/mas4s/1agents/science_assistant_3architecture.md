# 架构决断 Agent (Science Assistant: Architecture)

## 1. 概述 (Overview)

**Science Assistant: Architecture** 是专门用于科研课题可行性深度推演与技术架构决断的智能 Agent。其核心目标是在正式启动实验（阶段 4）之前，基于阶段 2 产出的文献底座（`literature_graph.json`），对拟定的研究架构进行全方位的软硬件雷区扫描、逻辑闭环验证及沙盘推演。
...
### 步骤 1：全域上下文解析 (Context Ingestion)

- **技能调用**：[`mas4s-architecture-context-ingestion`](file:///Users/admin/clawd/skills/mas4s-architecture-context-ingestion/SKILL.md) (参考：[技能文档](../1skills/mas4s_architecture_context_ingestion.md))
...
- **技能调用**：[`mas4s-architecture-logical-modeling`](file:///Users/admin/clawd/skills/mas4s-architecture-logical-modeling/SKILL.md) (参考：[技能文档](../1skills/mas4s_architecture_logical_modeling.md))
...
- **技能调用**：[`mas4s-architecture-reality-verification`](file:///Users/admin/clawd/skills/mas4s-architecture-reality-verification/SKILL.md) (参考：[技能文档](../1skills/mas4s_architecture_reality_verification.md))
- **输入**：`通用技术需求清单`。
...
- **技能调用**：[`mas4s-architecture-sandbox-deduction`](file:///Users/admin/clawd/skills/mas4s-architecture-sandbox-deduction/SKILL.md) (参考：[技能文档](../1skills/mas4s_architecture_sandbox_deduction.md))
- **输入**：`通用技术需求清单 + 约束条件`。
- **作用**：基于物理事实进行理论推演，识别性能瓶颈。
- **输出**：优缺点、风险矩阵、标准化降维方案 -> 传递至步骤 5。

### 步骤 5：架构决断报告 (Architecture Deduction Report)

- **技能调用**：[`mas4s-architecture-report`](file:///Users/admin/clawd/skills/mas4s-architecture-report/SKILL.md) (参考：[技能文档](../1skills/mas4s_architecture_report.md))
...
- **输出**：
  - `architecture_decision_report_cn.md`（综合决策报告 - 中文）
  - `architecture_decision_report_en.md`（综合决策报告 - 英文）
  - `architecture_decision_logic.json` / `architecture_decision_logic_cn.json` / `architecture_decision_logic_en.json`（下游逻辑载荷 - 双语/纯中文/纯英文）
  - `architecture_decision_visual.json` / `architecture_decision_visual_cn.json` / `architecture_decision_visual_en.json`（UI 可视化态势数据 - 双语/纯中文/纯英文）
...
所有 `mas4s-architecture-` 前缀的核心 Skill 必须满足以下规范：
