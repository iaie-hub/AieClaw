...
export function registerClawHubHandlers(handlers: SimpleHandlers, deps: ClawHubHandlersDeps): void {
  ...
  handlers["aiemas.clawhub.agenthub.visibility.update"] = async ({ params, respond }) => {
    try {
      ...
      const result = await proxyToRegistry("PUT", baseUrl, `/api/v1/clawhub/agent/${agentId}/visibility`, config.apiKey, undefined, {
        visibility,
      });
  ...
  handlers["aiemas.clawhub.agent.download"] = async ({ params, respond }) => {
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        const req = requestFn(
          ...
          (res) => {
            if (res.statusCode === 401) {
              reject(new ClawHubProxyError(CLAWHUB_ERROR_CODES.INVALID_API_KEY, "API Key invalid or expired", 401));
              return;
            }
            if (res.statusCode! < 200 || res.statusCode! >= 300) {
              reject(new ClawHubProxyError(CLAWHUB_ERROR_CODES.REGISTRY_ERROR, `AgentRegistry returned HTTP ${res.statusCode}`, res.statusCode));
              return;
            }
        ...
        req.on("error", (err) => {
          reject(new ClawHubProxyError(CLAWHUB_ERROR_CODES.SERVICE_UNREACHABLE, `Failed to download: ${err.message}`));
        });

        req.on("timeout", () => {
          req.destroy();
          reject(new ClawHubProxyError(CLAWHUB_ERROR_CODES.TIMEOUT, "Download request timed out (60s)"));
        });
